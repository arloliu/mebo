...
test: clean-test-results
	...
	@CGO_ENABLED=1 go test $(TEST_DIRS) -short -timeout=$(TEST_TIMEOUT) -race | tee -a test.log
	@! grep -q "^--- FAIL" test.log

	@echo "Run tests in parallel with CGO_ENABLED=0..."
	@CGO_ENABLED=0 go test $(TEST_DIRS) -short -timeout=$(TEST_TIMEOUT) | tee -a test.log
	@! grep -q "^--- FAIL" test.log
...
lint:
	@golangci-lint run --timeout 5m --timeout ${LINT_TIMEOUT}
