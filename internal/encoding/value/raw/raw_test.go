...
func TestRawValuesEach(t *testing.T) {
...
func TestRawNumericCodecContract(t *testing.T) {
	values := []float64{math.Inf(-1), -1.5, 0, math.Inf(1)}

	encoder := NewNumericRawEncoder(endian.GetBigEndianEngine())
	t.Cleanup(encoder.Finish)
	encoder.WriteSlice(values)

	safe := NewNumericRawDecoder(endian.GetBigEndianEngine())
	...
	require.Zero(t, safe.DecodeAll(encoder.Bytes()[:len(encoder.Bytes())-1], len(values), decoded))

	unsafeEncoder := NewNumericRawEncoder(endian.GetLittleEndianEngine())
	t.Cleanup(unsafeEncoder.Finish)
	unsafeEncoder.WriteSlice(values)
	unsafe := NewNumericRawUnsafeDecoder(endian.GetLittleEndianEngine())
	for index, want := range values {
		got, ok := unsafe.At(unsafeEncoder.Bytes(), index, len(values))
		require.True(t, ok)
		require.Equal(t, want, got)
	}
	require.Zero(t, unsafe.DecodeAll(unsafeEncoder.Bytes()[:len(unsafeEncoder.Bytes())-1], len(values), decoded))
}
