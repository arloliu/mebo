// Package raw implements fixed-width float64 encoding.
package raw
...
func (d NumericRawUnsafeDecoder) All(data []byte, count int) iter.Seq[float64] {
	return func(yield func(float64) bool) {
		...
		floatSlice, err := decodeFloat64SliceUnsafe(data[:count*8])
...
func decodeFloat64SliceUnsafe(data []byte) ([]float64, error) {
	if len(data)%8 != 0 {
...
// UnsafeDecodeFloat64Slice returns a zero-copy float64 view of aligned raw data.
//
// The caller must not retain the result after data is released or modified.
