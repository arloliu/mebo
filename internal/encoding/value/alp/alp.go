// Package alp implements adaptive lossless floating-point value compression.
package alp
...
// this is what makes the verify pass vectorizable (alp_encsimd_amd64.s).
...
func (d NumericALPDecoder) decodeMainInto(data []byte, count int, dst []float64) int {
	ee := int(data[0])                       //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	ff := int(data[1])                       //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	width := int(data[2])                    //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	nExc := int(d.engine.Uint32(data[3:7]))  //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	mn := int64(d.engine.Uint64(data[7:15])) //nolint:gosec
	codes := data[15:]                       //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	exc := data[15+(count*width+7)/8:]
	...
	for i := start; i < n; i++ {
		...
		var code uint64
		if bp+8 <= clen && sh+width <= 64 {
			code = (binary.LittleEndian.Uint64(codes[bp:bp+8]) >> sh) & mask //nolint:gosec // bp and clen guard the fast window read.
		} else {
			code = alpReadBitsSlow(codes, bitpos, width, mask)
		}
...
func (d NumericALPDecoder) decodeRDInto(data []byte, count int, dst []float64) int {
	rbw := int(data[0])                     //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	codeBits := int(data[1])                //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	nDict := int(data[2])                   //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	nExc := int(d.engine.Uint32(data[3:7])) //nolint:gosec // DecodeAll keeps the established trusted-column contract; blob validates ALP columns before dispatch.
	off := 7
	var dict [alpRDMaxDictSize]uint64 // ≤8 entries; stack-allocated
	for i := range nDict {
