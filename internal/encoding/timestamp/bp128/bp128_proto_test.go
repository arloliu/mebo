...
//   GOEXPERIMENT=simd go test ./internal/encoding/timestamp/bp128 -run 'BP128Proto' -v
//   GOEXPERIMENT=simd go test ./internal/encoding/timestamp/bp128 -run x -bench 'BP128Proto' -benchmem
...
func makeDods(blocks int, jitterPct float64) []uint64 {
	...
	for i := 1; i < len(ts); i++ {
		delta := ts[i] - prevTS
		out = append(out, bp128ZigZag(delta-prevDelta))
		prevTS = ts[i]
		prevDelta = delta
	}
...
func bpFullEncode(ts []int64, scratch []uint64) []uint64 {
	...
	for i := 1; i < len(ts); i++ {
		delta := ts[i] - prevTS
		scratch = append(scratch, bp128ZigZag(delta-prevDelta))
		prevTS = ts[i]
		prevDelta = delta
	}
	...
	b.Run("BP128_Encode_SIMDdod", func(b *testing.B) {
		...
		for b.Loop() {
			...
			for len(remaining) > 0 {
				...
				prevTS, prevDelta = deltadelta.IntoActive(deltaBuf[:c], remaining[:c], prevTS, prevDelta)
				for _, d := range deltaBuf[:c] {
					zz = append(zz, bp128ZigZag(d))
				}
