// Package delta implements delta-of-delta timestamp compression.
package delta
...
func (e *TimestampDeltaEncoder) WriteSlice(timestampsUs []int64) {
	...
	if deltadelta.ShouldUse(len(remaining)) {
...
func (d TimestampDeltaDecoder) All(data []byte, count int) iter.Seq[int64] {
	return func(yield func(int64) bool) {
		...
		for produced := 2; produced < count; produced++ {
			deltaZigzag, nextOffset, ok := varint.DecodeU64(data, offset)
			if !ok {
				return
			}
			offset = nextOffset

			deltaOfDelta := varint.DecodeZigZag64(deltaZigzag)
