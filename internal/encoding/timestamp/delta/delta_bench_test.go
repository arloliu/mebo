...
// BenchmarkTimestampRawEncoder_vs_Delta compares raw and delta encoding.
func BenchmarkTimestampRawEncoder_vs_Delta(b *testing.B) {
	...
	b.Run("raw_encoding", func(b *testing.B) {
		...
		for b.Loop() {
			encoder := tsraw.NewTimestampRawEncoder(endian.GetLittleEndianEngine())
...
// BenchmarkTimestampDeltaEncoder_RepeatedWrite benchmarks repeated writes.
...
// BenchmarkTimestampDeltaEncoder_RepeatedWrite_vs_WriteSlice compares write APIs.
		...
		for _, size := range sizes {
			b.Run(fmt.Sprintf("%s/%dpts", scenario.name, size), func(b *testing.B) {
				...
				b.Run("Raw", func(b *testing.B) {
					var finalSize int
					for b.Loop() {
						encoder := tsraw.NewTimestampRawEncoder(engine)
						encoder.WriteSlice(timestamps)
						finalSize = encoder.Size()
					}
