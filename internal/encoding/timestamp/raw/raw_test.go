...
func TestRawTimestampsEach(t *testing.T) {
...
func TestRawTimestampCodecContract(t *testing.T) {
	engine := endian.GetBigEndianEngine()
	values := []int64{-1, 0, 1, 1_725_000_000_123_456}
	...
	require.Equal(t, len(values), safe.DecodeAll(encoder.Bytes(), len(values), decoded))
	require.Equal(t, values, decoded)

	for index, want := range values {
		got, ok := safe.At(encoder.Bytes(), index, len(values))
		require.True(t, ok)
		require.Equal(t, want, got)
	}

	malformed := encoder.Bytes()[:len(encoder.Bytes())-1]
	require.Zero(t, safe.DecodeAll(malformed, len(values), decoded))

	unsafeEncoder := NewTimestampRawEncoder(endian.GetLittleEndianEngine())
	t.Cleanup(unsafeEncoder.Finish)
	unsafeEncoder.WriteSlice(values)
	unsafe := NewTimestampRawUnsafeDecoder(endian.GetLittleEndianEngine())
	for index, want := range values {
		got, ok := unsafe.At(unsafeEncoder.Bytes(), index, len(values))
		require.True(t, ok)
		require.Equal(t, want, got)
	}

	unsafeMalformed := unsafeEncoder.Bytes()[:len(unsafeEncoder.Bytes())-1]
	require.Zero(t, unsafe.DecodeAll(unsafeMalformed, len(values), decoded))
}

func TestUnsafeDecodeInt64SliceEmptyInput(t *testing.T) {
	values, err := UnsafeDecodeInt64Slice(nil)
	require.NoError(t, err)
	require.Empty(t, values)

	_, err = UnsafeDecodeInt64Slice([]byte{1})
	require.Error(t, err)
}
