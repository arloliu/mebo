...
//
// The encoder enforces a hard limit of 255 UTF-8 bytes per string.
...
//
// The string is validated to ensure it doesn't exceed MaxTextLength (255 bytes).
...
//
// Parameters:
//   - text: String to encode (must not exceed 255 UTF-8 bytes)
//
// Returns:
...
//
// All strings are validated to ensure none exceed MaxTextLength (255 UTF-8 bytes).
...
//
// Parameters:
//   - texts: Slice of strings to encode (each must not exceed 255 UTF-8 bytes)
//
// Returns:
